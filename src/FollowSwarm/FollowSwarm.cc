...
#include "FollowSwarm.h"
#include "Vehicle.h"
#include "PositionManager.h"

FollowSwarm::FollowSwarm(QGCApplication* app)
    : QGCTool(app), estimatation_capabilities(0)
{
    ...
    connect(&_gcsMotionReportTimer, &QTimer::timeout, this, &FollowSwarm::_sendGCSMotionReport);
}

FollowSwarm::~FollowSwarm()
{
    _disable();
}

void FollowSwarm::followSwarmHandleManager(const QString&)
{
    QmlObjectListModel & vehicles = *_toolbox->multiVehicleManager()->vehicles();

    for (int i=0; i< vehicles.count(); i++) {
        Vehicle* vehicle = qobject_cast<Vehicle*>(vehicles[i]);
        if (vehicle->px4Firmware() && vehicle->flightMode().compare(FirmwarePlugin::px4FollowSwarmFlightMode, Qt::CaseInsensitive) == 0) {
            _enable();
            return;
        }
    }
...
void FollowSwarm::_setGPSLocation(QGeoPositionInfo geoPositionInfo)
{
...
void FollowSwarm::_sendGCSMotionReport(void)
{
    QmlObjectListModel & vehicles = *_toolbox->multiVehicleManager()->vehicles();
    ...
    for (int i=0; i< vehicles.count(); i++) {
        Vehicle* vehicle = qobject_cast<Vehicle*>(vehicles[i]);
        if(vehicle->flightMode().compare(FirmwarePlugin::px4FollowSwarmFlightMode, Qt::CaseInsensitive) == 0) {
...
double FollowSwarm::_degreesToRadian(double deg)
{
